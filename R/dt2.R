...
if (interactive()) {
    ...
    N_REPS         <- 5L
    OUTFILE        <- file.path(root, "output", "dt2-3-months.rds")
} else {
...
cat(" - data.table benchmark 2: Sourcing helper functions and checking required repetitions\n")
source(INFILE_HELPERS)
reps <- as.integer(N_REPS)
if (is.na(reps))
    stop("Invalid value for the number of repetitions")

cat(" - data.table benchmark 2: Loading taxi data\n")
col_classes  <- readRDS(INFILE_CLASSES)
taxi_dat <- fread(INFILE_TAXI, colClasses = col_classes, na.strings = "", data.table = FALSE)
invisible(gc())

cat(sprintf(" - data.table benchmark 2: running %d iterations\n", reps))
res <- benchmark(gh2, n = reps, package = "data.table")

saveRDS(res, OUTFILE)

cat(" - data.table benchmark 2: Finished! \n\n")
